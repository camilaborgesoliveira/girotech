...
public class AnaliseGeralService {
    ...
    private static final Logger log = LoggerFactory.getLogger(AnaliseGeralService.class);
    ...
    public AnaliseGeralService(PedidoRepositorio pedidoRepositorio,
                               ClienteRepositorio clienteRepositorio,
                               ProdutoRepositorio produtoRepositorio,
                               AnaliseEstatisticaService analiseEstatisticaService) {
        this.pedidoRepositorio          = pedidoRepositorio;
        this.clienteRepositorio         = clienteRepositorio;
        this.produtoRepositorio         = produtoRepositorio;
        this.analiseEstatisticaService  = analiseEstatisticaService;
    }
    ...
    public GiroAnaliseGeralDTO analisarGeral() {
        ...
        // ── 2. Ticket médio e desvio padrão global ───────────────────
        ...
        // ── 3. HashMap: categoria → contagem de vendas ───────────────
        //    Uso de HashMap para busca O(1) por categoria
        Map<String, Long> vendasPorCategoria = new HashMap<>();
        for (Pedido p : todosPedidos) {
            String cat = p.getProduto().getCategoria().getNome();
            vendasPorCategoria.merge(cat, 1L, Long::sum);
        }

        // ── 4. Moda: produto e categoria mais vendidos ────────────────
        ...
        // ── 5. TreeSet: categorias em ordem alfabética ────────────────
        //    TreeSet mantém elementos sempre ordenados (O(log n) inserção)
        TreeSet<String> categoriasTree = new TreeSet<>(vendasPorCategoria.keySet());
        List<String> categoriasOrdenadas = new ArrayList<>(categoriasTree);

        // ── 6. LinkedList: histórico das últimas 10 compras ───────────
        //    LinkedList para inserção/remoção eficiente nas extremidades
        ...
        // ── 7. Teoria dos Conjuntos: segmentação de clientes ──────────
        //    Conjunto A = Premium, Conjunto B = Intermediário, Conjunto C = Econômico
        Map<String, Long> clientesPorSegmento = new HashMap<>();
        clienteRepositorio.findAll().forEach(cliente -> {
            List<Pedido> pedidosCliente = pedidoRepositorio
                .buscarPedidosComProdutoPorCliente(cliente.getId());
            if (!pedidosCliente.isEmpty()) {
                double media = calcularMedia(pedidosCliente);
                String segmento = media >= LIMIAR_PREMIUM       ? "Premium"
                                : media >= LIMIAR_INTERMEDIARIO ? "Intermediário"
                                                                : "Econômico";
                clientesPorSegmento.merge(segmento, 1L, Long::sum);
            }
        });

        // ── 8. Probabilidade Condicional P(Y | X) ─────────────────────
        //    "Dado que o cliente comprou categoria X, qual a prob. de comprar Y?"
    ...
    private List<ProbabilidadeDTO> calcularProbabilidades(List<Pedido> pedidos) {
        // Monta mapa: clienteId → Set<categorias compradas>
        Map<Long, Set<String>> comprasPorCliente = new HashMap<>();
        for (Pedido p : pedidos) {
            Long cid = p.getCliente().getId();
            comprasPorCliente
                .computeIfAbsent(cid, k -> new HashSet<>())
                .add(p.getProduto().getCategoria().getNome());
        }

        // Conta freq. de cada categoria (|A|)
        Map<String, Long> freqCategoria = new HashMap<>();
        comprasPorCliente.values().forEach(cats ->
            cats.forEach(cat -> freqCategoria.merge(cat, 1L, Long::sum))
        );

        // Conta co-ocorrências |A ∩ B|
        Map<String, Long> coOcorrencias = new HashMap<>();
        comprasPorCliente.values().forEach(cats -> {
            List<String> lista = new ArrayList<>(cats);
            for (int i = 0; i < lista.size(); i++) {
                for (int j = i + 1; j < lista.size(); j++) {
                    String chave1 = lista.get(i) + " → " + lista.get(j);
                    String chave2 = lista.get(j) + " → " + lista.get(i);
                    coOcorrencias.merge(chave1, 1L, Long::sum);
                    coOcorrencias.merge(chave2, 1L, Long::sum);
                }
            }
        });

        // Calcula P(Y|X) = |X∩Y| / |X| e ordena por probabilidade desc (MergeSort)
        return coOcorrencias.entrySet().stream()
            .map(e -> {
                ...
                return new ProbabilidadeDTO(
                    catX, partes[1], prob,
                    String.format("%.0f%% dos clientes que compraram '%s' também compraram '%s'",
    ...
    private double calcularDesvioPadrao(List<Pedido> pedidos, double media) {
        double variancia = pedidos.stream()
            .mapToDouble(p -> {
